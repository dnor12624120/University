...
class Repository
{
	public:
		...
		*/
		virtual Object getObject(const Key& key);

		/*
			Returns an iterator to the repository (map).
		*/
		virtual typename std::map<Key, std::unique_ptr<Object>>::iterator iterator();

		/*
			Returns an iterator to the beginning of the repository (map).
		*/
		virtual typename std::map<Key, std::unique_ptr<Object>>::iterator begin();

		/*
			Returns an iterator to the end of the repository (map).
		*/
		virtual typename std::map<Key, std::unique_ptr<Object>>::iterator end();

		/*
			Returns a map of copies of the records whose location is location,
			and their accessTime is less than timesAccessed.
		*/
		virtual std::vector<Object> filter(const std::function<bool(Object*)>& filterFunction);

		/*
			Returns the number of objects in the map.
		*/
		virtual inline const int size() const
		{
