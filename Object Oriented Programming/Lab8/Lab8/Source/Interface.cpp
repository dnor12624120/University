...
#define USER_LIST_LOCATION_INDEX 11
...
#define USER_LIST_LOCATION_ARGUMENT_COUNT 1
...
#define USER_LIST_LOCATION_FILEPATH_INDEX 1
	...
	m_inputValidator(std::unique_ptr<InputValidator>(new InputValidator(*this)))
{
	...
	m_commands.insert(std::make_pair<std::string, Command>("mylistLocation", Command("mylistLocation", USER_LIST_LOCATION_ARGUMENT_COUNT, USER_LIST_LOCATION_INDEX)));
	...
	m_functions[USER_LIST_LOCATION_INDEX] = std::bind(&Interface::userListLocation, this, std::placeholders::_1);
}
...
void Interface::userListLocation(const std::vector<std::string> &arguments)
{
	try
	{
		if (m_currentAccess == InterfaceAccess::ADMIN)
		{
			m_controller->setAdminListFilepath(arguments[USER_LIST_LOCATION_FILEPATH_INDEX]);
			m_controller->loadAdminListObjects();
		}
		else
		{
			m_controller->setUserListFilepath(arguments[USER_LIST_LOCATION_FILEPATH_INDEX]);
