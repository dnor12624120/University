...
class Controller
{
	public:
		...
		*/
		Controller(std::unique_ptr<Repository<std::string, Record>> repository, std::unique_ptr<FunctionStack> stack,
		...
		*/
		void deleteRecord(const std::string& title, bool add);
		...
		*/
		...
		std::map<std::string, Record>::iterator adminListBegin();

		std::map<std::string, Record>::iterator adminListEnd();
	...
	private:
		...
		std::unique_ptr<FunctionStack> m_userStack;
		std::unique_ptr<FunctionStack> m_adminStack;
