...
{
	return m_repository->getRecord(title);
}
...
std::map<std::string, Record> Controller::filterTimesAccessed(const std::string& location, const std::string& timesAccessed)
{
	return m_repository->filterTimesAccessed(location, timesAccessed);
}
