...
bool Interface::validateInput(const std::string& command, int argumentCount)
{
	...
	else
	{
		...
		while (iterator->first == command && iterator != m_commands.end() && !commandFound)
		{
...
void Interface::save(const std::vector<std::string> &arguments)
{
	try
	{
		Record record = m_controller->getRecord(arguments[1]);
...
void Interface::userList(const std::vector<std::string> &arguments)
{
	...
	{
		printRecord(record->second);
	}
}

void Interface::listTimesAccessed(const std::vector<std::string> &arguments)
{
	auto filteredRecords = m_controller->filterTimesAccessed(arguments[1], arguments[2]);
